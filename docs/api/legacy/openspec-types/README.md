**@astrospec/openspec-types**

***

# @astrospec/openspec-types

## Interfaces

- [AcceptanceCriterion](interfaces/AcceptanceCriterion.md)
- [BaseField](interfaces/BaseField.md)
- [DerivedSpec](interfaces/DerivedSpec.md)
- [EvidenceRef](interfaces/EvidenceRef.md)
- [EvidenceRetrievalMetadata](interfaces/EvidenceRetrievalMetadata.md)
- [EvidenceSpan](interfaces/EvidenceSpan.md)
- [JsonArray](interfaces/JsonArray.md)
- [JsonObject](interfaces/JsonObject.md)
- [JsonPatch](interfaces/JsonPatch.md)
- [AstroSpecBase](interfaces/AstroSpecBase.md)
- [AstroSpecContext](interfaces/AstroSpecContext.md)
- [AstroSpecContextRequestEnvelope](interfaces/AstroSpecContextRequestEnvelope.md)
- [AstroSpecRequestEnvelope](interfaces/AstroSpecRequestEnvelope.md)
- [AstroSpecTurn](interfaces/AstroSpecTurn.md)
- [AstroSpecTurnRequestEnvelope](interfaces/AstroSpecTurnRequestEnvelope.md)
- [ProvenanceEntry](interfaces/ProvenanceEntry.md)
- [ToolBinding](interfaces/ToolBinding.md)
- [VerificationReport](interfaces/VerificationReport.md)
- [VerifierResult](interfaces/VerifierResult.md)
- [VerifyEnv](interfaces/VerifyEnv.md)

## Type Aliases

- [AcceptanceCriteria](type-aliases/AcceptanceCriteria.md)
- [Context](type-aliases/Context.md)
- [FieldScope](type-aliases/FieldScope.md)
- [FieldSource](type-aliases/FieldSource.md)
- [FieldType](type-aliases/FieldType.md)
- [ISODateTime](type-aliases/ISODateTime.md)
- [JsonPatchOp](type-aliases/JsonPatchOp.md)
- [JsonPrimitive](type-aliases/JsonPrimitive.md)
- [JsonValue](type-aliases/JsonValue.md)
- [~~OpenSpecBase~~](type-aliases/OpenSpecBase.md)
- [~~OpenSpecContext~~](type-aliases/OpenSpecContext.md)
- [~~OpenSpecContextRequestEnvelope~~](type-aliases/OpenSpecContextRequestEnvelope.md)
- [~~OpenSpecRequestEnvelope~~](type-aliases/OpenSpecRequestEnvelope.md)
- [~~OpenSpecTurn~~](type-aliases/OpenSpecTurn.md)
- [~~OpenSpecTurnRequestEnvelope~~](type-aliases/OpenSpecTurnRequestEnvelope.md)
- [Provenance](type-aliases/Provenance.md)
- [SpecDiff](type-aliases/SpecDiff.md)
- [SpecField](type-aliases/SpecField.md)
- [Turn](type-aliases/Turn.md)
- [ValidationError](type-aliases/ValidationError.md)
- [VerifierFn](type-aliases/VerifierFn.md)

## Variables

- [SCHEMA\_IDS](variables/SCHEMA_IDS.md)

## Functions

- [applyPatches](functions/applyPatches.md)
- [assertContext](functions/assertContext.md)
- [assertTurn](functions/assertTurn.md)
- [canonicalizeSpec](functions/canonicalizeSpec.md)
- [canonicalizeValue](functions/canonicalizeValue.md)
- [diffSpecs](functions/diffSpecs.md)
- [lockSpec](functions/lockSpec.md)
- [mergeContextIntoTurn](functions/mergeContextIntoTurn.md)
- [registerVerifier](functions/registerVerifier.md)
- [signSpec](functions/signSpec.md)
- [stableStringify](functions/stableStringify.md)
- [validateContext](functions/validateContext.md)
- [validateTurn](functions/validateTurn.md)
- [validateWithSchema](functions/validateWithSchema.md)
- [verifyOutput](functions/verifyOutput.md)
