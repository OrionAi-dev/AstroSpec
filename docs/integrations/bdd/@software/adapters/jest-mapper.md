...
This adapter connects OpenSpec `@software` criteria to **Jest** by **referencing existing tests only**. It does **not** generate test code or embed testing semantics into OpenSpec. Instead, it scans your Jest test suite for lightweight annotations that link tests to OpenSpec `criteria.id` values and then produces a traceability index.

## Design principles

- **OpenSpec stays agnostic.** No Jest-specific fields in the OpenSpec schema.
...
- Reads one or more OpenSpec YAML files that declare:
  - `profile: https://orionai-dev.github.io/mcp-secure-context-sharing/profiles/@software`
  - `requirements[].criteria[].id`
- Scans your Jest tests for **OpenSpec annotations** and builds:
...
1. **Inline token in test title**
   - `it("creates a session [openspec:FEATURE-101.1.1]", () => { ... })`
...
3. **Describe-level tag to apply to all nested tests**
   - `describe("auth flows @openspec(FEATURE-101.1)", () => { ... })`
...
## Minimal example (OpenSpec → Jest reference)
...
```ts
// auth.login.spec.ts
describe("auth login @openspec(FEATURE-101.1)", () => {
  it("creates a session [openspec:FEATURE-101.1.1]", async () => {
    // arrange / act / assert
  });
});
```
...
- Each `criteria.id` found in OpenSpec may be:
...
- Prefer **single, unambiguous IDs** per test. If a test covers multiple criteria, list multiple tokens:
  - `it("login guards [openspec:SEC-200.1][openspec:SEC-200.2]", ...)`
- Keep titles human-readable; use square-bracket tokens or the `@openspec()` tag for machine parsing.
- If you practice TDD, consider tagging as soon as a test is drafted to avoid drift.

## Integration tips

- CI step order:
  1. Validate OpenSpec YAML against `@software` schema
...
- [ ] OpenSpec YAML validates and includes `criteria.id` values
- [ ] Tests include `@openspec(...)` or `[openspec:ID]` annotations
