...
import type { AcceptanceCriterion, EvidenceRef, JsonObject, AstroSpecContext, AstroSpecTurn } from "./types.js";
...
registerVerifier("json_schema", async ({ output, criterion, env }) => {
  ...
  const details = (validate.errors || [])
    .map((e: { instancePath?: string; message?: string }) => `${e.instancePath || "/"}: ${e.message || "schema violation"}`)
    .join("; ");
